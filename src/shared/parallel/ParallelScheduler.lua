...
function WorkerModule:SharedTableToTable(St : SharedTable)
	local Table = {}
	for i, v in St do
		v = typeof(v) == "SharedTable" and self:SharedTableToTable(v) or v
		Table[i] = v
	end
	return Table
end
...
function WorkerModule:LoadModule(ModuleScript : ModuleScript)
	...
	local function CreateWorker(ModuleScript : ModuleScript)
		local Worker = ReplicatedStorage.WorkerTemplate.Actor:Clone()

		local ModuleScript = ModuleScript:Clone()
		ModuleScript.Parent = Worker

		Worker.ServerScript.Enabled = not isClient
		Worker.ClientScript.Enabled = not not isClient
	...
	function ModuleTable:Work()
		...
		-- print("Scheduled tasks: "..ActiveWork.TableIndex)
		...
		for WorkerId, MergedResults in SharedResultsTable do 
			for i, v in MergedResults do 
				v = typeof(v) == "SharedTable" and WorkerModule:SharedTableToTable(v) or v
